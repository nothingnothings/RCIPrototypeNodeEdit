  ...
  $.fn.pekeUpload = function (options) {
    ...
    var pekeUpload = {
      ...
      replacehtml: function () {
        ...
        switch (options.dragMode) {
          case true:
            switch (options.bootstrap) {
              case true:
                html =
                  '<div class="well well-lg pkuparea pkdragarea" style="cursor:pointer"><h4>' +
                  options.dragText +
                  '</h4></div>';
                break;
              ...
              case false:
                html =
                  '<div class="pekeupload-drag-area pkuparea pkdragarea" style="cursor:pointer"><h4>' +
                  options.dragText +
                  '</h4></div>';
                break;
            }
          ...
          case false:
            switch (options.bootstrap) {
              case true:
                html =
                  '<a href="javascript:void(0)" class="btn btn-primary btn-upload pkuparea"> <i class="glyphicon glyphicon-upload"></i> ' +
                  options.btnText +
                  '</a>';
                break;
              ...
              case false:
                html =
                  '<a href="javascript:void(0)" class="pekeupload-btn-file pkuparea">' +
                  options.btnText +
                  '</a>';
                break;
            }
        ...
        this.container = $(
          '<div class="pekecontainer"><ul></ul></div>'
        ).insertAfter(this.uparea);
      },
      ...
      addWarning: function (error, c) {
        ...
        switch (options.bootstrap) {
          case true:
            html =
              '<div class="alert alert-danger"><button type="button" class="close pkwrncl" data-dismiss="alert">&times;</button> ' +
              error +
              '</div>';
            break;
          ...
          case false:
            html =
              '<div class="alert-pekeupload"><button type="button" class="close pkwrncl" data-dismiss="alert">&times;</button> ' +
              error +
              '</div>';
            break;
        }
      ...
      addRow: function (file) {
        ...
        switch (options.bootstrap) {
          case true:
            var newRow = $(
              '<div class="row pkrw" rel="' + i + '"></div>'
            ).appendTo(this.container);
            if (options.showPreview) {
              var prev = $(
                '<div class="col-lg-2 col-md-2 col-xs-4"></div>'
              ).appendTo(newRow);
            ...
            if (options.notAjax == false) {
              var progress = $(
                '<div class="progress"><div class="pkuppbr progress-bar" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="min-width: 2em;"></div></div>'
              ).appendTo(finfo);
            ...
            $(
              '<a href="javascript:void(0);" class="btn btn-danger pkdel">' +
                options.delfiletext +
                '</a>'
            ).appendTo(dismiss);
            break;
          ...
          case false:
            var newRow = $(
              '<div class="pekerow pkrw" rel="' + i + '"></div>'
            ).appendTo(this.container);
            if (options.showPreview) {
              var prev = $('<div class="pekeitem_preview"></div>').appendTo(
                newRow
              );
              this.previewFile(prev, file);
            }
            ...
            if (options.notAjax == false) {
              var progress = $(
                '<div class="progress-pekeupload"><div class="pkuppbr bar-pekeupload pekeup-progress-bar" style="min-width: 2em;width:0%"><span></span></div></div>'
              ).appendTo(finfo);
              if (options.showPercent) {
            ...
            $(
              '<a href="javascript:void(0);" class="delbutton pkdel">' +
                options.delfiletext +
                '</a>'
            ).appendTo(dismiss);
            break;
      ...
      previewFile: function (container, file) {
        ...
        switch (type) {
          case 'image':
            ...
            var prev = $(
              '<img class="thumbnail" src="' + fileUrl + '" height="64" />'
            ).appendTo(container);
            break;
          ...
          case 'video':
            var fileUrl = window.URL.createObjectURL(file);
            var prev = $(
              '<video src="' + fileUrl + '" width="100%" controls></video>'
            ).appendTo(container);
            break;
          ...
          case 'audio':
            var fileUrl = window.URL.createObjectURL(file);
            var prev = $(
              '<audio src="' + fileUrl + '" width="100%" controls></audio>'
            ).appendTo(container);
            break;

          default:
            if (options.bootstrap) {
              var prev = $('<i class="glyphicon glyphicon-file"></i>').appendTo(
                container
              );
            } else {
              var prev = $('<div class="pekeupload-item-file"></div>').appendTo(
                container
              );
            }
            break;
        ...
        $.ajax({
          ...
          complete: function () {
            if (options.onSubmit) {
              ...
              if (
                pekeUpload.uploadedfiles == pekeUpload.files.length &&
